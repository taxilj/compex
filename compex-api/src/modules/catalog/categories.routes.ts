...
import { PUBLIC_CACHE_CONTROL, cachedCatalog } from "./catalog-cache.js";
...
export async function categoriesRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/", async (_req, reply) => {
    const roots = await cachedCatalog("categories", async () => {
      // Only the fields the category UI renders (no timestamps).
