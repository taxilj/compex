...
describe("public manufacturers and category hierarchy", () => {
  it("paginates manufacturers server-side, cacheable, without internal source fields", async () => {
    const first = await app.inject({ method: "GET", url: "/api/v1/manufacturers?limit=1&page=1" });
    const second = await app.inject({ method: "GET", url: "/api/v1/manufacturers?limit=1&page=2" });
    expect(first.statusCode).toBe(200);
    expect(first.headers["cache-control"]).toContain("s-maxage");
    expect(first.json().data).toHaveLength(1);
    expect(first.json().meta.total).toBeGreaterThanOrEqual(2);
    expect(second.json().data[0].id).not.toBe(first.json().data[0].id);
    for (const secret of ["sourceUrl", "source", "isActive"]) expect(first.json().data[0]).not.toHaveProperty(secret);
    expect((await app.inject({ method: "GET", url: "/api/v1/manufacturers?limit=5000" })).statusCode).toBe(400);
  });

  it("keeps a three-level category tree intact", async () => {
    const parent = (await post("/api/v1/admin/categories", { name: "Tree L1" })).id;
    const child = (await post("/api/v1/admin/categories", { name: "Tree L2", parentId: parent })).id;
    await post("/api/v1/admin/categories", { name: "Tree L3", parentId: child });
    const roots = (await app.inject({ method: "GET", url: "/api/v1/categories" })).json().data as Array<{ name: string; children: Array<{ name: string; children: Array<{ name: string }> }> }>;
    const l1 = roots.find((c) => c.name === "Tree L1");
    expect(l1?.children[0].name).toBe("Tree L2");
    expect(l1?.children[0].children[0].name).toBe("Tree L3");
  });
});
