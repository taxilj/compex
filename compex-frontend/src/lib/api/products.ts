...
// One bounded retry for idempotent public reads: a timeout, network failure or
// 5xx (typically a cold-starting backend) gets a second attempt; caller
// aborts and 4xx are never retried.
async function retryRead<T>(run: () => Promise<T>): Promise<T> {
  try {
    return await run();
  } catch (err) {
    const retryable =
      (err instanceof ApiError && (err.code === "TIMEOUT" || err.statusCode >= 500)) || err instanceof TypeError;
