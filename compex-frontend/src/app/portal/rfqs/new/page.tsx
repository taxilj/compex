...
import { ChevronRight, ChevronLeft, Plus, Trash2, Upload, CheckCircle } from "lucide-react";
...
export default function NewRFQPage() {
  ...
  const [bomFile, setBomFile] = useState<string | null>(null);
  ...
  const addComp = () =>
    setComps((prev) => [
      ...prev,
      { id: `c${Date.now()}`, mpn: "", manufacturer: "", quantity: "", targetPrice: "" },
    ]);
          ...
          return (
            <div key={label} className="flex items-start flex-1 min-w-0">
              <div className="flex flex-col items-center gap-1 flex-shrink-0">
                ...
                <span
                  className={`font-label-sm text-xs text-center ${
                    active ? "text-[#0B1F3A] font-medium" : "text-[#44474d]"
                  }`}
                >
                  {label}
                </span>
              ...
              {i < STEP_LABELS.length - 1 && (
                <div
                  className={`flex-1 h-0.5 mt-4 mx-1 ${done ? "bg-[#12B76A]" : "bg-[#E4E7EC]"}`}
                />
              )}
            </div>
          );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={req.contact}
                  onChange={(e) => setReq((r) => ({ ...r, contact: e.target.value }))}
                  className={INPUT_CLS}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={req.gstin}
                  onChange={(e) => setReq((r) => ({ ...r, gstin: e.target.value }))}
                  className={INPUT_CLS}
                />
              </div>
            </div>
          ...
          <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <label className="flex items-center gap-2 px-3 py-1.5 border border-[#E4E7EC] rounded font-label-sm text-[#44474d] cursor-pointer hover:border-[#1769E0] hover:text-[#1769E0] transition-colors text-sm">
                  ...
                  <input
                    ...
                    accept=".csv,.xlsx,.xls"
                    className="hidden"
                    onChange={(e) => setBomFile(e.target.files?.[0]?.name ?? null)}
                  />
                </label>
                <button
                  onClick={addComp}
                  className="flex items-center gap-2 px-3 py-1.5 bg-[#0B1F3A] text-white rounded font-label-sm hover:bg-[#1a3357] transition-colors text-sm"
                >
            ...
            {bomFile && (
              <div className="bg-[#f0f3ff] border border-[#1769E0]/30 rounded p-3 font-body-sm text-[#0B1F3A]">
                BOM uploaded: <strong>{bomFile}</strong>
              </div>
            )}
              ...
              <table className="w-full min-w-[580px]">
                <thead>
                  <tr className="bg-[#f0f3ff]">
                    {["Part Number", "Manufacturer", "Quantity", "Target Price (₹)", ""].map((h) => (
                      <th
                        key={h}
                        className="py-2.5 px-3 text-left font-label-sm text-[#44474d] uppercase tracking-wider text-xs"
                      >
                        {h}
                      </th>
                    ))}
                    ...
                    <tr key={c.id}>
                      <td className="py-2 px-3">
                        <input
                          type="text"
                          value={c.mpn}
                          onChange={(e) => updateComp(c.id, "mpn", e.target.value)}
                          placeholder="e.g. STM32F103"
                          className="w-full px-2 py-1.5 border border-[#E4E7EC] rounded font-body-sm text-[#111c2d] focus:outline-none focus:border-[#1769E0]"
                        />
                      </td>
                      <td className="py-2 px-3">
                        <input
                          ...
                          onChange={(e) => updateComp(c.id, "manufacturer", e.target.value)}
                      ...
                      <td className="py-2 px-3">
                        <input
                          type="number"
                          value={c.quantity}
                          onChange={(e) => updateComp(c.id, "quantity", e.target.value)}
                          className="w-full px-2 py-1.5 border border-[#E4E7EC] rounded font-body-sm text-[#111c2d] focus:outline-none focus:border-[#1769E0]"
                        />
                      </td>
                      <td className="py-2 px-3">
                        <input
                          type="number"
                          value={c.targetPrice}
                          onChange={(e) => updateComp(c.id, "targetPrice", e.target.value)}
                          className="w-full px-2 py-1.5 border border-[#E4E7EC] rounded font-body-sm text-[#111c2d] focus:outline-none focus:border-[#1769E0]"
                        />
                      </td>
                      <td className="py-2 px-3">
                        <button
                          ...
                          className="p-1.5 text-[#44474d] hover:text-[#F04438] disabled:opacity-30 transition-colors rounded"
                        >
                          <Trash2 size={16} />
                        </button>
                      </td>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  type="date"
                  value={delivery.requiredDate}
                  onChange={(e) => setDelivery((d) => ({ ...d, requiredDate: e.target.value }))}
                  className={INPUT_CLS}
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setDelivery((d) => ({ ...d, priority: e.target.value }))}
                  className={INPUT_CLS}
                >
              ...
              <div className="md:col-span-2">
                <label className="block font-label-sm text-[#44474d] mb-1.5">
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Certifications, special requirements, or notes..."
                  className="w-full px-3 py-2.5 border border-[#E4E7EC] rounded font-body-md text-[#111c2d] focus:outline-none focus:border-[#1769E0] resize-none"
                />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-[#f9f9ff] rounded-lg p-4">
                <h3 className="font-label-sm text-[#44474d] uppercase tracking-wider text-xs mb-3">
                  Company
                </h3>
                <div className="space-y-2">
                  {[
                    { k: "Company", v: req.company },
                    { k: "Contact", v: req.contact },
                    { k: "Email", v: req.email },
                    { k: "Phone", v: req.phone },
                    { k: "GSTIN", v: req.gstin },
                  ]
              ...
              <div className="bg-[#f9f9ff] rounded-lg p-4">
                ...
                <div className="space-y-2">
                  {[
                    { k: "Location", v: delivery.location },
                    { k: "Date", v: delivery.requiredDate },
                    { k: "Priority", v: delivery.priority },
                  ]
                    .filter((r) => r.v)
                    .map((r) => (
                      <div key={r.k} className="flex gap-2 font-body-sm">
            ...
            <div>
              <h3 className="font-label-sm text-[#44474d] uppercase tracking-wider text-xs mb-3">
                Components ({comps.filter((c) => c.mpn).length})
              </h3>
              <div className="overflow-x-auto">
                    ...
                    {comps
                      ...
                      .map((c) => (
                        <tr key={c.id}>
                          ...
                          <td className="py-2.5 px-4 font-body-sm text-[#111c2d]">
                            {c.manufacturer}
                          </td>
                          <td className="py-2.5 px-4 font-mono-label text-[#111c2d]">
                            {c.quantity}
                          </td>
                          <td className="py-2.5 px-4 font-mono-label text-[#111c2d]">
                            {c.targetPrice ? `₹${c.targetPrice}` : "—"}
                          </td>
                        </tr>
                      ))}
                  </tbody>
        ...
        {step === 5 && (
          <div className="flex flex-col items-center text-center gap-6 py-8">
            ...
            <div>
              ...
              <p className="font-body-md text-[#44474d] mt-2">
                Your RFQ has been submitted. Our team will review and respond within 24 hours.
              </p>
            </div>
            <div className="flex gap-3">
              <Link
                href="/portal/rfqs"
                className="bg-[#0B1F3A] text-white px-6 py-2.5 rounded font-label-md hover:bg-[#1a3357] transition-colors"
              >
                View My RFQs
              </Link>
              <Link
                href="/portal"
                className="border border-[#E4E7EC] text-[#44474d] px-6 py-2.5 rounded font-label-md hover:bg-[#f0f3ff] transition-colors"
              >
                Dashboard
              </Link>
            </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-3">
            {step > 1 && (
              <button
                onClick={back}
                className="flex items-center gap-2 px-4 py-2 border border-[#E4E7EC] text-[#44474d] rounded font-label-md hover:bg-[#f0f3ff] transition-colors"
              >
            ...
            <Link
              href="/portal/rfqs"
              className="px-4 py-2 text-[#44474d] font-label-md hover:text-[#111c2d] transition-colors"
            >
              Cancel
            </Link>
          </div>
          <div className="flex gap-3">
            <button className="px-4 py-2 border border-[#E4E7EC] text-[#44474d] rounded font-label-md hover:bg-[#f0f3ff] transition-colors">
              Save Draft
            </button>
            <button
              onClick={next}
              className="flex items-center gap-2 bg-[#0B1F3A] text-white px-6 py-2 rounded font-label-md hover:bg-[#1a3357] transition-colors"
            >
              {step === 4 ? "Submit RFQ" : "Continue"} <ChevronRight size={16} />
            </button>
          </div>
        </div>
      )}
